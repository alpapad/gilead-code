...
import net.sf.gilead.sample.client.command.RefreshMessageListCommand;
...
public class NewMessagePanel extends Composite
{
	 ...
	 */
	protected Message _lastCreateMessage;
	...
	//	IMPORTANT : must use the GWT.create method
	//
		Message message = (Message) GWT.create(Message.class);
		message.setAuthor(ApplicationParameters.getInstance().getUser());
	...
	//
		MessageRemote.Util.getInstance().saveMessage(message, new DefaultCallback(){
			...
			//
				_lastCreateMessage = (Message)result;
